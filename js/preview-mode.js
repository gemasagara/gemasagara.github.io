// js/preview-mode.js
// This file runs on preview.html and loads data from localStorage instead of GitHub
...
class PreviewApp {
  ...
  async init() {
    if (this.initialized) {
      logInfo('PreviewApp already initialized');
      return;
    }

    try {
      logInfo('Initializing PreviewApp in preview mode...');
      
      // Enable preview mode in dataLoader
      dataLoader.setPreviewMode(true);
      ...
      logInfo('PreviewApp initialized successfully (preview mode)');
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => previewApp.init());
} else {
  previewApp.init();
}
...
if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
  window.portfolioApp = previewApp;
  console.log('💡 Preview mode: Access app via window.portfolioApp');
}
